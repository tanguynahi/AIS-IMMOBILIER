        ...
        <div class="comment @if ($n==$nb) comment-b @endif">
            <div class="comment-author">
                <a>
                    ...
                    <img src="@if($value->EST_RECEPTEUR===Help::$E) {{ asset($photo ?? 'boy_icon-icons.png') }}
                    @else {{ asset($entreprise->LOGO ?? '4341274_120573.png') }}  @endif">
                </a>
            </div>
            <div class="comment-content">
                <h5>
                    @if($value->EST_RECEPTEUR===Help::$C)
                    <span style="color: grey; font-size: 14px">{{ $entreprise->RAISON_SOCIALE ?? 'BMI-WFS' }}</span>
                    @else {{$value->NOM_PRENOMS ?? 'xxxxx xxxxxxx'}} @endif
                </h5>
