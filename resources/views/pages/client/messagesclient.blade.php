...
@section('content')
    @include('partials.client.breadcrumb-navcli', ['LBL' => 'Messages'])
                            ...
                            <div class="table-responsive mb-1">
                                <table class="table align-middle table-nowrap" id="customerTable">
                                    ...
                                    <tbody class="list form-check-all">
                                        ...
                                        <tr>
                                            <td class="id">
                                                <a href="{{route('messageDetailCli',['id'=>$value->ID_CONTACT])}}"
                                                    @if ($value->NB_NON_LU>0) style="font-weight: bold;" @endif>
                                                    @if ($value->ADR_EMAIL===$us->ADR_EMAIL && $value->CONTACT!='')
                                                    <span style="color: grey; font-size: 14px; font-weight: bold;">MOI</span>
                                                    @else {{ $entreprise->RAISON_SOCIALE ?? "BMI-WFS" }}  @endif
                                                </a>
                                            </td>
                                            ...
                                            <td>
                                                <div class="comment-meta">
                                                    <a href="{{route('messageDetailCli',['id'=>$value->ID_CONTACT])}}">Consulter</a>
                                                </div>
                                            </td>
                                        </tr>
                                        @endforeach
                        ...
                        <div class="comment @if ($n==$nb) comment-b @endif">
                            ...
                            <div class="comment-content">
                                <div class="comment-meta">
                                    <h6>
                                        @if ($value->ADR_EMAIL===$us->ADR_EMAIL && $value->CONTACT!='')
                                        <span style="color: grey; font-size: 14px">MOI</span>
                                        @else BHCI @endif
                                    </h6>
                                    <div class="">
...
@section('js')
    <script>
        function form() {
            var rout = "{{route('messageNouveauCli')}}";
